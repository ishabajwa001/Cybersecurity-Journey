...
```text
100000 packets transmitted
99628 received
0.372% packet loss

rtt min/avg/max/mdev =
5.057 / 9.225 / 187.415 / 6.772 ms
...
- Average round-trip time was approximately **9.225 ms**.
- Maximum latency reached **187.415 ms**, indicating occasional buffering or congestion during heavy traffic.
- The **mdev** value of **6.772 ms** indicates moderate variation in response times throughout the test.
- The **ipg (Inter-Packet Gap)** of **8.584 ms** represents the average spacing between transmitted packets. Although flood mode attempts maximum transmission speed, the actual rate is limited by CPU scheduling, kernel timing, network hardware, and destination processing speed.
- The **pipe** value of **12** indicates the approximate number of ICMP requests that were simultaneously outstanding during transmission.
...
| `sudo ping -f -c 100000 1.0.0.1` | Root | Allowed | 100000 | 0.372% | High-rate ICMP transmission completed successfully |
